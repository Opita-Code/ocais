...
export async function generateObject(options) {
    const { provider, model, system, prompt, schema, temperature, signal: userSignal, timeoutMs, onStart, onComplete, onError, onAbort, } = options;
    ...
    try {
        ...
        if (typeof zodSchema._def?.toJsonSchema === "function") {
            ...
            try {
                ...
                if (mod?.zodToJsonSchema) {
                    jsonSchema = mod.zodToJsonSchema(zodSchema);
                }
                else {
        ...
        try {
            ...
            throw new Error(`OCAIS: Failed to parse AI response as structured object: ${err instanceof Error ? err.message : "Unknown error"}. Raw: ${response.content.slice(0, 200)}`);
        }
