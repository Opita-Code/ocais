 ...
 */
import { base64UrlEncode, base64UrlDecode, constantTimeEqual, hmacSha256, } from './utils.js';
import { AuthCookieInvalidError, AuthError, } from './errors.js';
...
async function aesGcmEncrypt(key, iv, plaintext, aad) {
    ...
    const ct = await globalThis.crypto.subtle.encrypt({ name: "AES-GCM", iv: iv, additionalData: aad, tagLength: 128 }, cryptoKey, plaintext);
...
async function aesGcmDecrypt(key, iv, ciphertext, aad) {
    ...
    const pt = await globalThis.crypto.subtle.decrypt({ name: "AES-GCM", iv: iv, additionalData: aad, tagLength: 128 }, cryptoKey, ciphertext);
 ...
 */
export async function cookieSign(value, options) {
 ...
 *   tampered, or expired.
 */
export async function cookieVerify(cookieValue, options) {
...
function formatSetCookieHeader(value, attrs, maxAge) {
    const parts = [`ocais=${value}`];
    if (attrs.path)
 ...
 */
export function buildDeleteCookieHeader(name, attrs = {}) {
    const parts = [`${name}=`];
    if (attrs.path)
        parts.push(`Path=${attrs.path}`);
    else
 ...
 */
export function inspectCookie(cookieValue) {
    let raw = cookieValue;
    if (raw.includes("=")) {
        const eq = raw.indexOf("=");
        raw = raw.substring(eq + 1).split(";")[0];
