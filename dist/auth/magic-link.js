...
import { AuthError, AuthMagicLinkInvalidError, AuthRateLimitError, AuthStorageError, } from './errors.js';
 ...
 */
export async function magicLinkRequest(payload, options) {
    const { storage, secret, ttlMs = 600_000, // 10 min per OWASP
    rateLimitMs = 60_000, // 1 per minute
    tokenBytes = 32, } = options;
    if (ttlMs < 60_000) {
 ...
 */
export async function magicLinkVerify(args, options) {
