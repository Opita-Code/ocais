...
async function getArgon2() {
    ...
    try {
        ...
        throw new AuthError("AUTH_DEPENDENCY", "passwordHash/passwordVerify require @node-rs/argon2 to be installed (npm install @node-rs/argon2)");
    }
}
/**
 ...
 */
export async function passwordHash(password, options = {}) {
    const params = {
 ...
 */
export async function passwordVerify(hash, password) {
    const argon2 = await getArgon2();
    try {
 ...
 */
export async function paramsFromHash(hash) {
 ...
 */
export async function needsRehash(hash, recommended) {
