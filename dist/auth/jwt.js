 ...
 */
import { randomBase64Url, base64UrlEncode, base64UrlDecode, hmacSha256, constantTimeEqual, } from './utils.js';
import { AuthTokenInvalidError, AuthTokenExpiredError, AuthTokenNotYetValidError, AuthTokenAudienceError, AuthTokenIssuerError, AuthStorageError, AuthError, } from './errors.js';
// ─── HS256 implementation (zero-dep) ────────────────────────────────────
/**
 * Computes an HS256 signature. Returns the signature as base64url.
 */
async function hs256Sign(signingInput, secret) {
 ...
 */
async function hs256Verify(signingInput, signature, secret) {
    const cryptoKey = await globalThis.crypto.subtle.importKey("raw", secret, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    const computed = new Uint8Array(await globalThis.crypto.subtle.sign("HMAC", cryptoKey, new TextEncoder().encode(signingInput)));
 ...
 */
async function rs256Sign(signingInput, privateKey) {
    const cryptoKey = await globalThis.crypto.subtle.importKey("pkcs8", privateKey, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["sign"]);
    const sig = await globalThis.crypto.subtle.sign({ name: "RSASSA-PKCS1-v1_5" }, cryptoKey, new TextEncoder().encode(signingInput));
 ...
 */
async function rs256Verify(signingInput, signature, publicKey) {
    const cryptoKey = await globalThis.crypto.subtle.importKey("spki", publicKey, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["verify"]);
    return globalThis.crypto.subtle.verify({ name: "RSASSA-PKCS1-v1_5" }, cryptoKey, signature, new TextEncoder().encode(signingInput));
}
/**
 ...
 */
export async function rs256VerifyJWK(signingInput, signature, jwk) {
    const cryptoKey = await globalThis.crypto.subtle.importKey("jwk", { kty: "RSA", n: jwk.n, e: jwk.e, alg: "RS256" }, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["verify"]);
    return globalThis.crypto.subtle.verify({ name: "RSASSA-PKCS1-v1_5" }, cryptoKey, signature, new TextEncoder().encode(signingInput));
}
...
async function getEd25519() {
    ...
    try {
        ...
        throw new AuthError("AUTH_DEPENDENCY", "EdDSA requires @noble/ed25519 to be installed (npm install @noble/ed25519)");
    }
}
...
function decodeHeader(headerB64) {
    try {
        return JSON.parse(base64UrlDecodeString(headerB64));
    }
    catch {
 ...
 */
export async function signJWT(claims, options) {
...
export async function verifyJWT(token, options) {
    const { storage, audience, issuer, algorithms = ["HS256"], clockSkewSec = 5, rejectExpired = true, } = options;
    ...
    if (!header.alg || !algorithms.includes(header.alg)) {
        throw new AuthTokenInvalidError(`JWT alg "${header.alg}" is not in the allow-list: ${algorithms.join(", ")}`);
    }
    ...
    if (audience) {
        ...
        if (!hasMatch) {
            throw new AuthTokenAudienceError(Array.isArray(audience) ? audience.join(",") : audience, Array.isArray(claims.aud) ? claims.aud.join(",") : (claims.aud ?? "(none)"));
        }
    }
    return claims;
}
...
export async function rotateKeys(options) {
    ...
    if (alg === "HS256") {
        ...
        const keyPair = await globalThis.crypto.subtle.generateKey({ name: "RSASSA-PKCS1-v1_5", modulusLength: 2048, publicExponent: new Uint8Array([1, 0, 1]), hash: "SHA-256" }, true, ["sign", "verify"]);
        // Export private key as PKCS#8 DER bytes
        secretOrPrivate = new Uint8Array(await globalThis.crypto.subtle.exportKey("pkcs8", keyPair.privateKey));
        // Export public key as SPKI DER bytes
        publicKey = new Uint8Array(await globalThis.crypto.subtle.exportKey("spki", keyPair.publicKey));
    }
    else {
        throw new AuthError("AUTH_CONFIG", `Unsupported algorithm: ${alg}`);
    }
...
export async function jwksPublish(options) {
    ...
    for (const kid of keyIds) {
        ...
        if (alg === "EdDSA") {
            ...
            const cryptoKey = await globalThis.crypto.subtle.importKey("spki", stored.publicKey, { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, true, ["verify"]);
 ...
 */
export function futureSec(seconds) {
