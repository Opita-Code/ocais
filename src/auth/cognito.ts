...
const _cache = new Map<string, JWKCache>();
const _pendingFetches = new Map<string, Promise<JWKCache>>();

function _cacheKey(region: string, userPoolId: string): string {
  return `${region}/${userPoolId}`;
}

function _jwksUrl(region: string, userPoolId: string): string {
...
async function _fetchJWKS(
  ...
  const promise = (async () => {
    ...
    const cache: JWKCache = { keys: data.keys, fetchedAt: Date.now() };
...
async function _getKeyByKid(
  ...
  if (!retry) {
    throw new AuthTokenInvalidError(
      `JWK with kid "${kid}" not found after refresh. Available kids: ${
        (_cache.get(key)?.keys ?? []).map((k) => k.kid).join(", ")
      }`,
    );
  }
  return retry;
 ...
 */
export function _resetCache(region?: string, userPoolId?: string): void {
...
export function _cacheStats(): {
  poolCount: number;
  totalKeys: number;
  pools: Array<{ region: string; userPoolId: string; keyCount: number; ageMs: number }>;
} {
  const pools: Array<{ region: string; userPoolId: string; keyCount: number; ageMs: number }> = [];
  let totalKeys = 0;
...
export async function verifyCognitoJWT(
  ...
  if (!header.alg || header.alg !== "RS256") {
    throw new AuthTokenInvalidError(
      `JWT alg must be "RS256", got "${header.alg ?? "(none)"}"`,
    );
  }
  ...
  if (tokenUse && payload.token_use !== tokenUse) {
    throw new AuthTokenInvalidError(
      `token_use mismatch: expected "${tokenUse}", got "${payload.token_use}"`,
    );
  }
  ...
  if (payload.iss && payload.iss !== expectedIss) {
    throw new AuthTokenInvalidError(
      `Issuer mismatch: expected "${expectedIss}", got "${payload.iss}"`,
    );
  }
  ...
  if (tokenUse === "id" && clientId) {
    ...
    if (!audList.includes(clientId)) {
      throw new AuthTokenInvalidError(
        `Audience mismatch: expected "${clientId}", got "${aud}"`,
      );
    }
    ...
    if (payload.client_id !== clientId) {
      throw new AuthTokenInvalidError(
        `client_id mismatch: expected "${clientId}", got "${payload.client_id}"`,
      );
    }
  }
 ...
 */
export async function _refreshJWKS(
  region: string,
  userPoolId: string,
  fetchFn?: typeof globalThis.fetch,
): Promise<void> {
